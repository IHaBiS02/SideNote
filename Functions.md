...
- `DEFAULT_PINNED_NOTE_DRAG_DELAY_MS`: Default pinned-note hold delay (`350ms`)
...
- `initializeWysiwygMarkdownEditor()`: Installs the 4.1.14-compatible SideNote Preview theme, shared semantic body CSS, shared Preview/Edit padding, code line numbers, the saved empty-paragraph preference, image hooks, pasted-text processing, and highlight.js decorations on `#markdown-editor`
...
- `createNoteContentStyles(options)`: Generates scoped semantic Markdown body
  ...
  alignment representation. Typography, headings, lists, blockquotes, links,
  inline code, checkbox geometry/accent, images, tables, and highlight.js token
  colors therefore stay synchronized without sharing editor chrome or document
  layout rules.
...
- `normalizePinnedNoteDragDelay(value, fallback)`: Normalizes the global-only
  pinned-note hold delay to an integer from 100 to 2000ms, defaulting to 350ms
...
- `createStandaloneNoteHtml(note)`: Parses and sanitizes Markdown, applies the
  shared Preview body styles and code highlighting, embeds SideNote-managed and
...
- `STANDALONE_NOTE_CSS`: Scoped light/dark document styling shared by both
  standalone formats; its note body is generated by
  `createNoteContentStyles()` while page layout remains export-specific
...
- `createPinnedNoteDragController(list, onReorder, options)`: Distinguishes a
  short click/scroll from a configurable long press (350ms by default) and
