...
-   **`src/`**: This directory contains all the JavaScript logic for the extension, broken down into modules.
    ...
    -   `editor/`:
        -   `sidenote-editor-adapter.js`: Connects the reusable `<wysiwyg-markdown>` component to SideNote image storage, paste processing, editor modes, and highlight.js syntax token ranges.
...
-   **`wysiwyg-markdown.js`**: Self-contained Lit/ProseMirror-based Web Component bundle generated by the parent editor project and checked in so SideNote release builds remain standalone. Code tokens use the existing local highlight.js runtime and Atom One color variables without replacing editable DOM.

The editor bundle is refreshed from the parent repository with `npm.cmd run sync:sidenote`. Its dependency notices are stored in `WYSIWYG_MARKDOWN_LICENSES.md` and copied into Chrome and Firefox builds.
