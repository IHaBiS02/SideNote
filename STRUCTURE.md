...
-   **`wysiwyg-markdown.js`**: Self-contained Lit/ProseMirror-based Web Component bundle generated by the parent editor project and checked in so SideNote release builds remain standalone. Code tokens use the existing local highlight.js runtime and Atom One color variables without replacing editable DOM. Multi-line code uses a separate non-editable gutter whose `pre > code` line boxes and padding mirror the editable code body; single-line code hides it so copied and saved code remains clean.
