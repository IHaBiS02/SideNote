...
-   **`packages/wysiwyg-markdown/`**: The reusable Lit/ProseMirror editor npm workspace:
    ...
    -   `src/element/wysiwyg-markdown.ts`: Form-associated Lit custom element, ProseMirror lifecycle, source modes, node views, events, image hooks, public API, middle-click link navigation, and stable read-only-to-editable code-language controls outside document content.
