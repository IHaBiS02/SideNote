...
-   **`wysiwyg-markdown.js`**: Self-contained Lit/ProseMirror-based Web Component bundle generated by the parent editor project and checked in so SideNote release builds remain standalone. Code tokens use the existing local highlight.js runtime and Atom One color variables without replacing editable DOM. Line numbers use a separate non-editable gutter so copied and saved code remains clean.
