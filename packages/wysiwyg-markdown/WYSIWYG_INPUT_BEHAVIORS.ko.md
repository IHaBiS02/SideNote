...
| 이미지 모달에서 `Ctrl+휠` 또는 터치패드 핀치 | 포인터 위치를 기준으로 연속 확대·축소하며, 고해상도 터치패드의 작은 입력도 반응하도록 감도를 정규화 |
| 모달 이미지를 누른 채 드래그 | 현재 위치에서 이미지를 이동 |
...
- 모달 배경이나 `Escape`로 이미지 모달을 닫으면 확대 배율과 이동 위치를
  버립니다. 이미지를 다시 열면 화면에 맞춘 중앙 상태에서 시작합니다.
