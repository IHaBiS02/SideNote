...
Attribute selectors apply `--editor-table-aligned-cell-padding` only on the
aligned edge of explicit left and right cells; its reusable default is `5px`.
...
SideNote exposes separate global and note-specific controls for WYSIWYG,
full-document plain-text, and fenced-code line spacing. Each accepts `1.0`
through `3.0` in `0.1` increments and writes only its corresponding component
host variables, so the three surfaces remain independently configurable.
