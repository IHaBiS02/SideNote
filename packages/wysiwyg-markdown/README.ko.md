...
WYSIWYG에서 붙여넣은 텍스트의 HTTP(S) URL은 자동 링크로 변환하며 코드와
기존 링크는 보존합니다. 직접 입력하거나 소스에서 일반 텍스트로 바꾼 URL은
문서 로딩·모드 전환 시 자동으로 다시 링크가 되지 않습니다.
...
렌더링된 링크는 가운데 버튼 클릭, `Ctrl+클릭`, `Cmd+클릭`으로 새 브라우저
탭에서 열립니다. 수정 키 없는 기본 클릭은 편집 가능한 문서 조작에 그대로
사용됩니다.

SideNote는 `displaySource`를 사용해 화면에 맞춘 이미지를 모달 중앙에 엽니다.
모달 이미지 클릭은 배율을 바꾸지 않습니다. `Ctrl+휠`과 브라우저가 터치패드
핀치에서 합성한 wheel 이벤트는 포인터를 기준으로 확대·축소하고, 터치스크린의
두 Pointer 간 거리와 움직이는 중심점은 직접 핀치 확대와 이동에 사용됩니다.
마우스 또는 한 손가락으로 누른 채 드래그해 이동할 수도 있습니다. 배경이나
`Escape`로 닫으면 제스처 상태가 정리되므로 다시 열었을 때 초기화됩니다.
...
작업 체크박스에서 발생한 더블클릭은 체크박스 조작으로 남으므로 빠르게 두 번
눌러도 source 모드가 열리지 않습니다.
