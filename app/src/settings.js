...
import { markdownEditor, autoLineBreakButton, tildeReplacementButton, showTildeReplacementButtonCheckbox, legacyLineBreakModeCheckbox, titleSetting, fontSizeSetting, lineHeightSetting, sourceLineHeightSetting, codeLineHeightSetting, pinnedNoteDragDelaySetting, pinnedNoteDragDelayContainer, modeSetting, codeBlockHeaderCheckbox, wysiwygPreviewCheckbox, preventExtraEmptyParagraphsCheckbox, preventExtraEmptyParagraphsContainer, preventUsedImageDeletionCheckbox } from './dom.js';
// Import state from state module
import { globalSettings, setActiveNoteId } from './state.js';
import { DEFAULT_PINNED_NOTE_DRAG_DELAY_MS, MAX_PINNED_NOTE_DRAG_DELAY_MS, MIN_PINNED_NOTE_DRAG_DELAY_MS, } from './constants.js';
...
function normalizeGlobalSettings(settings = {}) {
    ...
    delete normalizedSettings.autoAddSpaces;
    normalizedSettings.lineHeight = normalizeLineHeight(normalizedSettings.lineHeight);
    normalizedSettings.sourceLineHeight = normalizeLineHeight(normalizedSettings.sourceLineHeight, DEFAULT_SOURCE_LINE_HEIGHT);
    normalizedSettings.codeLineHeight = normalizeLineHeight(normalizedSettings.codeLineHeight, DEFAULT_CODE_LINE_HEIGHT);
    normalizedSettings.pinnedNoteDragDelayMs = normalizePinnedNoteDragDelay(normalizedSettings.pinnedNoteDragDelayMs);
    return normalizedSettings;
}
function normalizeLineHeight(value, fallback = DEFAULT_LINE_HEIGHT) {
...
function normalizePinnedNoteDragDelay(value, fallback = DEFAULT_PINNED_NOTE_DRAG_DELAY_MS) {
    const numericValue = typeof value === 'number' ? value : Number(value);
    if (!Number.isFinite(numericValue))
        return fallback;
    return Math.round(Math.min(MAX_PINNED_NOTE_DRAG_DELAY_MS, Math.max(MIN_PINNED_NOTE_DRAG_DELAY_MS, numericValue)));
}
function resolveEffectiveSettings(note) {
    ...
    if (noteSettings.title !== undefined)
        effectiveSettings.title = noteSettings.title;
    if (noteSettings.fontSize !== undefined)
        effectiveSettings.fontSize = noteSettings.fontSize;
    if (noteSettings.lineHeight !== undefined) {
        effectiveSettings.lineHeight = normalizeLineHeight(noteSettings.lineHeight, effectiveSettings.lineHeight);
    }
    if (noteSettings.sourceLineHeight !== undefined) {
        effectiveSettings.sourceLineHeight = normalizeLineHeight(noteSettings.sourceLineHeight, effectiveSettings.sourceLineHeight);
    }
    if (noteSettings.codeLineHeight !== undefined) {
        effectiveSettings.codeLineHeight = normalizeLineHeight(noteSettings.codeLineHeight, effectiveSettings.codeLineHeight);
    }
...
function applySourceLineHeight(lineHeight) {
...
function applyCodeLineHeight(lineHeight) {
...
function applyLineHeightSettings(settings) {
...
function applyMode(mode) {
    const followsDarkSystemTheme = mode === 'system'
...
function populateSettingsForm(isGlobal, note) {
    ...
    if (isGlobal) {
        ...
        pinnedNoteDragDelaySetting.value = String(effectiveGlobalSettings.pinnedNoteDragDelayMs);
...
// Export functions
export { DEFAULT_SETTINGS, MIN_LINE_HEIGHT, MAX_LINE_HEIGHT, MIN_PINNED_NOTE_DRAG_DELAY_MS, MAX_PINNED_NOTE_DRAG_DELAY_MS, normalizeGlobalSettings, normalizeLineHeight, normalizePinnedNoteDragDelay, resolveEffectiveSettings, resolveLegacyTextProcessingSettings, saveGlobalSettings, applyFontSize, applyLineHeight, applySourceLineHeight, applyCodeLineHeight, applyLineHeightSettings, applyMode, updateAutoLineBreakButton, updateTildeReplacementButton, updateLegacyLineBreakControls, isCodeBlockHeaderEnabled, populateSettingsForm };
