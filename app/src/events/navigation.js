...
import { backButton, settingsBackButton, licenseBackButton, recycleBinBackButton, imageManagementBackButton, markdownEditor } from '../dom.js';
// Import functions from other modules
import { sortNotes } from '../notes.js';
import { showListView, showSettingsView, showLicenseView, showRecycleBinView, showImageManagementView, openNote, renderNoteList } from '../notes_view/index.js';
...
import { notes, isGlobalSettings, originalNoteContent, setIsGlobalSettings, getLoadedNote, hydrateNote, } from '../state.js';
import { moveBack, getCurrentHistoryState, getHistory, getHistoryIndex, goToHistoryState } from '../history.js';
...
async function navigateToState(state) {
    ...
    switch (state.view) {
        ...
        case 'editor':
            ...
            await openNote(state.params.noteId, state.params.inEditMode ?? false, false);
            break;
        case 'settings':
...
function showHistoryDropdown(_targetButton) {
    const history = getHistory();
    if (history.length === 0)
        return;
    createDropdown({
...
// Export functions
export { navigateToState, goBack, populateHistoryDropdown, showHistoryDropdown, refreshHistoryDropdown, initializeNavigationEvents };
