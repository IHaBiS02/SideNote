import { newNoteButton, editorTitle, markdownEditor, toggleViewButton } from '../dom.js';
import { sortNotes } from '../notes.js';
import { openNote, getPreviewModeButtonLabel, togglePreview, showImageModal, renderNoteList } from '../notes_view/index.js';
...
import { notes, activeNoteId, isPreview, getLoadedNote, setIsPreview } from '../state.js';
function initializeEditorEvents() {
    ...
    markdownEditor.addEventListener('mode-change', (event) => {
        ...
        if (mode !== 'source' && mode !== 'wysiwyg' && mode !== 'readonly')
            return;
        const nextIsPreview = mode !== 'source';
        if (nextIsPreview === isPreview)
            return;
    ...
    markdownEditor.addEventListener('image-activate', (event) => {
        const { displaySource } = event.detail;
        if (displaySource)
            showImageModal(displaySource);
    });
