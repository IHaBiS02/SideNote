...
import { imageList, markdownEditor } from '../dom.js';
// Import required functions from other modules
import { getAllImageObjectsFromDB, getAllNotes, deleteImage, } from '../database/index.js';
import { createBlobUrlTracker, extractImageIds } from '../utils.js';
import { normalizeGlobalSettings } from '../settings.js';
// Import state from state module
import { notes, globalSettings } from '../state.js';
...
async function renderImagesList() {
    ...
    try {
        ...
        const storedNotes = (await getAllNotes()).filter(note => activeNoteIds.has(note.id) && !note.metadata.deletedAt);
                    ...
                    if (!isAlreadyOpen) {
                        ...
                        notesUsingImage.forEach(note => {
                            ...
                            noteItem.onclick = async () => {
                                if (!await openNote(note.id, false))
                                    return;
...
// Export functions
export { showImageModal, renderImagesList };
