...
function findPinnedNoteItem(list, target) {
    if (!(target instanceof Element))
        return null;
    const item = target.closest('li[data-note-id][data-pinned="true"]');
    return item && list.contains(item) ? item : null;
}
function pinnedNoteIds(list) {
    return Array.from(list.querySelectorAll('li[data-pinned="true"]')).map(item => item.dataset.noteId).filter((id) => Boolean(id));
}
function movePinnedPlaceholderAtPointer(list, drag, clientY, dropHysteresisPx) {
    ...
    // genuinely changed.
    if (nextDropIndex === drag.dropIndex)
        return;
    const otherPinnedItems = Array.from(list.querySelectorAll('li[data-pinned="true"]')).filter(candidate => candidate !== drag.item);
    const insertBefore = otherPinnedItems[nextDropIndex];
    if (insertBefore) {
        list.insertBefore(drag.placeholder, insertBefore);
    }
    else {
        const firstUnpinnedItem = list.querySelector('li[data-pinned="false"]');
        list.insertBefore(drag.placeholder, firstUnpinnedItem);
    }
    drag.dropIndex = nextDropIndex;
}
function clearFloatingStyles(item) {
 ...
 */
function createPinnedNoteDragController(list, onReorder, options = {}) {
    ...
    let candidate = null;
    let activeDrag = null;
    let suppressedClickNoteId = null;
    ...
    const activateCandidate = () => {
        ...
        const pinnedItems = Array.from(list.querySelectorAll('li[data-pinned="true"]'));
        ...
        placeholder.style.setProperty('--pinned-note-placeholder-height', `${bounds.height}px`);
        ...
        activeDrag = {
            ...
            grabOffsetY: Math.min(bounds.height, Math.max(0, current.startY - bounds.top)),
            dropIndex,
            dropBoundariesY,
        ...
        current.item.style.width = `${Math.max(0, bounds.width - FLOATING_HORIZONTAL_INSET_PX * 2)}px`;
    ...
    const cancelActiveDrag = () => {
        ...
        if (!cancelledDrag)
            return;
        activeDrag = null;
        if (cancelledDrag.originalNextSibling
    ...
    const handlePointerMove = (event) => {
        ...
        movePinnedPlaceholderAtPointer(list, activeDrag, event.clientY, dropHysteresisPx);
    };
    ...
    const handleClick = (event) => {
        ...
        const item = event.target instanceof Element
            ? event.target.closest('li[data-note-id]')
            : null;
        if (item?.dataset.noteId !== suppressedClickNoteId)
            return;
        suppressedClickNoteId = null;
        event.preventDefault();
    ...
    const handleTouchMove = (event) => {
        if (activeDrag && event.cancelable)
            event.preventDefault();
    };
...
export { createPinnedNoteDragController, };
