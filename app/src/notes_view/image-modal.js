...
function wheelZoomFactor(event) {
    ...
    if (delta === 0)
        return 1;
    let exponent = clamp(-delta * WHEEL_ZOOM_SPEED, -MAX_WHEEL_ZOOM_STEP, MAX_WHEEL_ZOOM_STEP);
...
function getPinchSnapshot(touchPointers) {
...
function showImageModal(blobUrl) {
    ...
    const toModalPoint = (clientPoint) => {
    ...
    const zoomBetween = (fromClientPoint, toClientPoint, factor) => {
    ...
    const finishDrag = (pointerId) => {
        if (!dragging || pointerId !== dragPointerId)
            return;
        dragging = false;
        dragPointerId = -1;
        modalImg.style.cursor = 'grab';
    };
    const finishTouchPointer = (pointerId) => {
        if (!touchPointers.has(pointerId))
            return;
        touchPointers.delete(pointerId);
        pinchSnapshot = null;
        const remainingPointer = touchPointers.entries().next().value;
        if (remainingPointer) {
        ...
        if (event.pointerType === 'touch') {
            ...
            if (touchPointers.size >= 2) {
                ...
                if (pinchSnapshot
                    ...
                    zoomBetween(pinchSnapshot.center, nextSnapshot.center, nextSnapshot.distance / pinchSnapshot.distance);
                }
...
export { closeImageModal, showImageModal, };
