...
function topLevelNoteFolders(zip) {
...
async function readSnotesManifest(zip) {
    const manifestFile = zip.file('manifest.json');
    if (!manifestFile)
        return null;
    const parsed = JSON.parse(await manifestFile.async('string'));
    if (parsed.formatVersion !== SNOTES_FORMAT_VERSION
...
async function parseSnotesArchive(zip) {
    ...
    let nextArchiveOrder = parsedNotes.reduce((maximum, note) => Math.max(maximum, note.archiveOrder ?? -1), -1) + 1;
    for (const folderName of availableFolders) {
        ...
        const parsedNote = await parseSnote(folder);
        if (manifest)
            parsedNote.archiveOrder = nextArchiveOrder++;
        parsedNotes.push(parsedNote);
    }
    return parsedNotes;
}
...
function importedAfterPinOrder(note) {
    const order = note.pinOrder ?? note.pinnedAt;
    return Number.isFinite(order) ? Number(order) : -1;
}
async function saveImportedNotes(parsedNotes, existingNotes = []) {
    const hasArchiveOrder = parsedNotes.some(note => (Number.isFinite(note.archiveOrder)));
    ...
    const maximumExistingPinOrder = existingNotes
        ...
        .reduce((maximum, note) => Math.max(maximum, importedAfterPinOrder(note)), -1);
    const maximumExistingLastModified = existingNotes.reduce((maximum, note) => Math.max(maximum, note.metadata.lastModified), 0);
...
// Export the function
export { parseSnote, parseSnotesArchive, saveParsedSnoteImages, createNoteFromParsedSnote, saveParsedSnote, processSnote, saveImportedNotes, getExportContent, createNoteFolderName, addNoteToZip, createSingleNoteArchive, createAllNotesArchive };
