...
고정 노트가 먼저 나오며 각 그룹 안에서는 JSON 배열 순서를 유지합니다. `id`는 영문 소문자·숫자·하이픈을
...
추가·가져오기·삭제는 비활성입니다. 핀은 클릭하거나 키보드로 바꿀 수 있고,
즉시 고정 노트가 위로 정렬됩니다. 읽고 있던 본문은 유지합니다.
핀 변경은 메모리에만 보관하므로 새로고침하면 게시자의 원래 설정으로 돌아옵니다.
