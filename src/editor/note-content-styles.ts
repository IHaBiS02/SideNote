interface NoteContentStyleOptions {
  rootSelector: string;
  variableNamespace: 'editor' | 'export';
  taskItemSelector: string;
  taskContentSelector?: string;
  checkedTaskSelector?: string;
  tableAlignmentSource: 'style' | 'align';
}
 ...
 */
function createNoteContentStyles({
  rootSelector: root,
  variableNamespace: namespace,
  taskItemSelector,
  taskContentSelector,
  checkedTaskSelector,
  tableAlignmentSource,
}: NoteContentStyleOptions): string {
  const variable = (name: string): string => `var(--${namespace}-${name})`;
...
export type { NoteContentStyleOptions };
