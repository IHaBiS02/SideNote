...
- `autoAddSpaces`: legacy option to add Markdown line break spaces when pressing Enter.
- `showTildeReplacementButton`: independent option to show the tilde replacement toolbar button.
- `tildeReplacement`: independent toolbar option to escape `~` on paste.
